...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // Deliverable 3: 1. Create a variable that filters the metadata array for the object with the desired sample number.
      var dataMetadate =data.metadata;
   
    // Deliverable 3: 2. Create a variable that holds the first sample in the metadata array.
    var metadatasArray = dataMetadata.filter(sampleObj => sampleObj.id == sample); 
        var metadata = metadataArray[0];
      console.log(metadataArray);
    // Deliverable 1: 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
      var otuIds = result.otu_ids;
      //console.log(outIds);
      var outLabels = result.out_labels;
    ...
    var yticks = outIds.slice(0,10).map(outIds => 'OTU ${outIds}').reverse();
    //console.log(yticks);
    ...
    // Deliverable 3  Plotly.newPlot("gauge", gaugeData, gaugeLayout);
